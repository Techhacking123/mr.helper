-- =====================================================
-- ORDER EXPIRY FINE SYSTEM - V2 (Fixed for existing database)
-- =====================================================
-- This migration safely handles existing objects
-- =====================================================

-- Step 1: Drop existing functions if they exist
DROP FUNCTION IF EXISTS get_provider_unpaid_fines(UUID);
DROP FUNCTION IF EXISTS pay_fine(UUID, UUID[], TEXT);
DROP FUNCTION IF EXISTS pay_fines_with_subscription(UUID, TEXT);
DROP FUNCTION IF EXISTS check_and_mark_expired_orders();
DROP FUNCTION IF EXISTS extend_order_deadline(UUID);
DROP FUNCTION IF EXISTS set_order_deadline();
DROP FUNCTION IF EXISTS update_provider_fines();
...
-- Step 3: Add Fine Tracking Columns to orders table
...
-- Step 4: Add Fine Tracking to users table (for providers)
...
-- Step 5: Create Fine Transactions Table
...
-- Step 6: Function to Update Provider Total Fines
...
-- Step 7: Function to Set Initial Deadline (on order creation)
...
-- Step 8: Update existing orders to have deadlines
UPDATE orders
SET 
...
-- Step 9: RPC Function to Extend Order Deadline (when "Working" clicked)
...
-- Step 10: RPC Function to Check & Mark Expired Orders
...
-- Step 11: RPC Function to Get Provider's Unpaid Fines
...
-- Step 12: RPC Function to Pay Fine (separate payment)
...
-- Step 13: RPC Function to Pay Fines with Subscription
...
-- Step 14: Grant permissions for RPC functions
...
-- Step 15: RLS Policies for fine_transactions table
...
-- Step 16: Create view for provider fine summary
...
GROUP BY u.id, u.name, u.total_unpaid_fines, u.total_fines_paid;
